...
namespace Overlay {
	...
	void initD3D(HWND hWnd)
	{
		...
		D3DXCreateFontA(d3ddev, 15, 0, TEXT_FONT, 1, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Arial", &pFont);
	...
	//util function for world to screen
	...
	void inline PlayerEsp() {
		...
		for (vector<Entity>::iterator it = pGame->players.begin(); it != pGame->players.end(); ++it) {

			Vector3 spos = WorldToScreen(it->pos, pGame->cameracache);
			
			//Don't show if the distance larger that PLAYER_FAR
			if (it->distance > PLAYER_FAR)
				continue;

			if (it->distance <= PLAYER_NEAR)
				color = COLOR_PLAYER_NEAR;
			else if (it->distance > PLAYER_NEAR && it->distance <= PLAYER_NORMAL)
				color = COLOR_PLAYER_NORMAL;
			else
				color = COLOR_PLAYER_FAR;

			DrawString((int)spos.x, (int)spos.y, color, pFont, "[%3d : %3d]", (int)it->distance, (int)it->health);
			DrawHealthBar((int)spos.x, (int)spos.y + 16, (int)it->health);
		}
